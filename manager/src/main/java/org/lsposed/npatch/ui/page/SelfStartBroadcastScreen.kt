...
// isAutoStart highlight accent (purple reads consistently in light/dark without the alarm
// connotation of MaterialTheme.colorScheme.error).
private val AUTO_START_ACCENT = Color(0xFF7E57C2)

/** Level 4A: broadcast (receiver) control for one app — master switch + per-receiver toggles. */
@OptIn(ExperimentalMaterial3Api::class)
@Destination
@Composable
fun SelfStartBroadcastScreen(packageName: String, navigator: DestinationsNavigator) {
    ...
    Scaffold(
        topBar = {
            SelfStartBackTopBar(stringResource(R.string.self_start_broadcast_control)) { navigator.navigateUp() }
        }
    ) { innerPadding ->
        Column(
            modifier = Modifier
